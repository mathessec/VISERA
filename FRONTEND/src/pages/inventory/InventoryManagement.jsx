...
import { Search, Plus, ArrowRight, ArrowRightLeft, Edit, Trash2 } from 'lucide-react';
...
import { getAllInventory, deleteInventoryStock, transferStock, updateInventoryQuantity } from '../../services/inventoryService';
...
export default function InventoryManagement() {
  ...
  const handleDelete = async (row) => {
    if (!window.confirm(`Are you sure you want to delete this inventory record? This will remove ${row.quantity} units of ${row.skuCode} from ${row.binName}.`)) {
      return;
    }
        ...
        <div>
          <div className="text-sm text-gray-900">
            {row.zoneName && row.rackName ? (
              <>
                {row.zoneName} <ArrowRight className="inline w-3 h-3 mx-1" /> {row.rackName} <ArrowRight className="inline w-3 h-3 mx-1" /> {row.binName}
              </>
            ) : (
              row.binName || '-'
            )}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">Manage inventory stock across all locations</p>
        </div>
